...
def train(tensor_text, tensor_face, batchsize):
    ...
    for num_batch in range(batchsize):
        text_loss_size += tensor_text[1][1][num_batch].numpy()
        face_loss_size += tensor_face[1][1][num_batch].numpy()
        #   隐藏层初始化
        ...
        if use_teacher_forcing:
            ...
            for di in range(tensor_text[1][1][num_batch]):
                ...
                text_loss += Criterion_text_glo(detext_output, tensor_text[1][0][num_batch][di])
    ...
    return text_loss.item()/text_loss_size
